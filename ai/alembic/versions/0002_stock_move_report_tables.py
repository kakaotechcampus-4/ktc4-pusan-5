"""add stock_move_report tables

(첫 줄은 `alembic history` 가 제목으로 찍는다. 한국어 Windows 콘솔에서 깨지므로
영어로 쓰고, 설명은 아래에 한국어로 적는다.)

우리가 LLM 으로 생성한 종목 변동 요인 보고서 네 표를 만든다. 설계 근거는 `app/models/stock_move_report.py`
...
def upgrade() -> None:
    ...
    op.create_index("ix_stock_move_report_ticker_as_of", "stock_move_reports", ["ticker", "as_of"])
    op.create_index("ix_stock_move_report_target_date", "stock_move_reports", ["target_date"])
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(["report_id"], ["stock_move_reports.id"], ondelete="CASCADE"),
        sa.PrimaryKeyConstraint("id"),
        sa.UniqueConstraint("report_id", "order_index", name="uq_stock_move_report_factor_order"),
    )
    op.create_index(
        "ix_stock_move_report_factors_report_id", "stock_move_report_factors", ["report_id"]
    )

    op.create_table(
        "stock_move_report_factor_sources",
        sa.Column("id", sa.BigInteger(), autoincrement=True, nullable=False),
        ...
        sa.ForeignKeyConstraint(
            ["factor_id"], ["stock_move_report_factors.id"], ondelete="CASCADE"
        ),
        sa.PrimaryKeyConstraint("id"),
        sa.UniqueConstraint("factor_id", "order_index", name="uq_stock_move_report_source_order"),
    )
    op.create_index(
        "ix_stock_move_report_factor_sources_factor_id",
        "stock_move_report_factor_sources",
        ["factor_id"],
    )
    op.create_index("ix_stock_move_report_source_url", "stock_move_report_factor_sources", ["url"])

    # 이번 PR 은 표만 만들고 쓰지 않는다. AI 서버(적재)와 검수 쪽이 같은 행을
    # 건드리면 단방향이 깨져서 보고서 표와 갈라 뒀다.
    op.create_table(
        "stock_move_report_reviews",
        sa.Column("id", sa.BigInteger(), autoincrement=True, nullable=False),
        sa.Column("report_id", sa.BigInteger(), nullable=False),
    ...
    op.create_index(
        "ix_stock_move_report_review_report", "stock_move_report_reviews", ["report_id"]
    )


def downgrade() -> None:
    op.drop_index("ix_stock_move_report_review_report", table_name="stock_move_report_reviews")
    op.drop_table("stock_move_report_reviews")
    op.drop_index("ix_stock_move_report_source_url", table_name="stock_move_report_factor_sources")
    op.drop_index(
        "ix_stock_move_report_factor_sources_factor_id",
        table_name="stock_move_report_factor_sources",
    )
    op.drop_table("stock_move_report_factor_sources")
    op.drop_index("ix_stock_move_report_factors_report_id", table_name="stock_move_report_factors")
    op.drop_table("stock_move_report_factors")
    op.drop_index("ix_stock_move_report_ticker_as_of", table_name="stock_move_reports")
    op.drop_index("ix_stock_move_report_target_date", table_name="stock_move_reports")
    op.drop_index(
        "ix_stock_move_report_passed",
        table_name="stock_move_reports",
        postgresql_where=sa.text("verify_status = 'passed'"),
    )
    op.drop_table("stock_move_reports")
