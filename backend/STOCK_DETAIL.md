...
한쪽 실패는 다른 쪽 정상값을 지우지 않는다. 최초의 정상 빈 응답은 24시간 캐시한다.
기존 데이터가 있는데 빈 응답이 오면 실패로 기록하고 저장된 값을 보존한다. 각 영역은
소유권이 확인된 트랜잭션에서 반환된 결산월만 갱신하여 정정을 반영하며, 누락된 과거 기간은
삭제하지 않는다. 저장된 최신 결산월보다
...
재무 응답이 비어 있으면 기존 기간 데이터가 있는 경우 `EMPTY_RESPONSE` 실패로 기록하고 기존
스냅샷을 보존한다. 기존 데이터가 없는 최초의 빈 응답만 정상적인 빈 캐시로 저장한다. 비어 있지
않은 응답은 반환된 결산월만 upsert하므로, 원천이 생략한 과거 기간을 삭제하지 않으며 명시적인
null 정정값은 그대로 반영한다.
