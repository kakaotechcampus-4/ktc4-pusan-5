...
def test_paragraph_break_counts_as_a_sentence_end() -> None:
    """마침표 없이 끝나는 소제목에서 안 끊으면 세 문장이 본문 절반을 삼킨다."""
    body = "1. 유가\n유가가 올랐다. 정유주가 강세였다."
    assert split_sentences(body)[0] == "1. 유가"


def test_excerpt_is_always_the_verbatim_head() -> None:
...
def test_over_the_cap_falls_back_to_a_sentence_boundary() -> None:
...
def test_no_sentence_end_returns_the_whole_body() -> None:
    body = "마침표가 없는 한 줄짜리 본문"
    assert first_sentences(body, 3, 400) == body
    assert sentence_ends(body) == []


def test_no_boundary_and_over_the_cap_cuts_at_the_cap() -> None:
