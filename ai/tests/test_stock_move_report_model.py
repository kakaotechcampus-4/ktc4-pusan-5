...
def test_표_이름이_backend_report_와_겹치지_않는다() -> None:
    ...
    assert StockMoveReportReview.__tablename__ == "stock_move_report_reviews"

    collisions = {"report", "report_block", "report_citation", "analyst_reports"}
    assert collisions.isdisjoint(
        {
            StockMoveReport.__tablename__,
            StockMoveReportFactor.__tablename__,
            StockMoveReportFactorSource.__tablename__,
            StockMoveReportReview.__tablename__,
        }
    )


def test_자연키에_unique_를_걸지_않는다() -> None:
    ...
    t = Base.metadata.tables["stock_move_reports"]
    ...
    assert "ix_stock_move_report_ticker_as_of" in {i.name for i in t.indexes}


def test_원본_json_은_비울_수_없다() -> None:
    """스키마 위반·파싱 실패 케이스가 반드시 나온다. 정규화 칼럼이 비더라도
    원본이 남아 있어야 원인을 되짚을 수 있다."""
    assert not Base.metadata.tables["stock_move_reports"].columns["raw_json"].nullable


def test_정규화_칼럼은_전부_비어도_된다() -> None:
    ...
    cols = Base.metadata.tables["stock_move_reports"].columns
    for name in (
        "ticker",
        "name",
        "target_date",
...
def test_검증_필드는_화면에_안_나가도_DB_에_있다() -> None:
    """quote / match / is_market_recap / not_found 는 서비스 화면에 나가지 않지만
    사후 환각 추적 수단이 이것뿐이다. 노출 제어는 backend 응답 단계에서 한다."""
    src = Base.metadata.tables["stock_move_report_factor_sources"].columns
    assert {"quote", "match", "is_market_recap"} <= set(src.keys())
    assert "not_found" in Base.metadata.tables["stock_move_reports"].columns


def test_출처_종류_칼럼이_지금_있다() -> None:
    """지금은 전부 telegram 이지만 뉴스·리포트가 붙을 때 소스별 오탐률을 갈라 봐야
    한다. 그때 추가하면 이미 쌓인 데이터가 전부 미상이 된다.
    이름은 analyst_reports.source 관례를 따랐다."""
    col = Base.metadata.tables["stock_move_report_factor_sources"].columns["source"]
    assert not col.nullable
    assert col.default.arg == "telegram"


def test_미검증이_기본값이다() -> None:
...
def test_검수_판정은_보고서_표에_붙지_않는다() -> None:
    """AI 서버(적재)와 검수 쪽이 같은 행을 건드리면 단방향이 깨진다. 보고서는
    INSERT only 로 두고 판정은 별도 표에 쌓는다."""
    report_cols = set(Base.metadata.tables["stock_move_reports"].columns.keys())
    assert "decision" not in report_cols
    assert "reviewer" not in report_cols
    assert "decision" in Base.metadata.tables["stock_move_report_reviews"].columns


def test_래퍼의_중복_지표는_담지_않는다() -> None:
    ...
    cols = set(Base.metadata.tables["stock_move_reports"].columns.keys())
    assert cols.isdisjoint({"n_factors", "n_sources", "duration_ms", "cost_usd"})


def test_시각_칼럼은_전부_timezone_aware_다() -> None:
